...
trait Lealez_Unified_Location_Metabox_Trait {
    ...
    private function render_embedded_metabox( $post_id, $definition ) {
            ...
            if ( ! $box || empty( $box['callback'] ) || ! is_callable( $box['callback'] ) ) {
                return '<div class="lealez-empty"><h3>' . esc_html__( 'No se pudo cargar el módulo', 'lealez' ) . '</h3><p>' . esc_html__( 'El metabox original no está registrado en esta instalación.', 'lealez' ) . '</p></div>';
            }
            ...
            <form id="post" class="lealez-gmb-metabox-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                ...
                <input type="hidden" name="module" value="<?php echo esc_attr( $this->definition_key_from_id( $definition['metabox_id'] ) ); ?>">
                ...
                <?php if ( ! empty( $definition['read_only'] ) ) : ?>
                    <div class="lealez-gmb-readonly-note"><span class="dashicons dashicons-lock"></span><p><?php esc_html_e( 'La vinculación se muestra en modo consulta. Para evitar asociaciones accidentales, solo el administrador del sitio puede cambiarla desde el backend.', 'lealez' ); ?></p></div>
                    <?php if ( $this->is_site_admin() ) : ?><a class="lealez-btn lealez-btn-secondary" href="<?php echo esc_url( get_edit_post_link( $post_id ) ); ?>"><?php esc_html_e( 'Editar vinculación en backend', 'lealez' ); ?></a><?php endif; ?>
