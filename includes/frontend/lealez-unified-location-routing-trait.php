...
trait Lealez_Unified_Location_Routing_Trait {
    ...
    private function enqueue_common_assets() {
        ...
        wp_localize_script(
            ...
            array(
                'googleLabel' => __( 'Publica en Google', 'lealez' ),
                'localLabel'  => __( 'Solo Lealez', 'lealez' ),
    ...
    public function replace_portal_output( $output, $tag, $attr, $match ) {
        ...
        if ( 'lealez_location_editor' === $tag ) {
            ...
            // A Google push requires an existing local location. The first
            // creation step remains local and then opens the unified profile.
        ...
        if ( 'lealez_location_google_center' === $tag ) {
            ...
            return '<div class="lealez-portal"><div class="lealez-empty"><h3>' . esc_html__( 'El perfil de ubicación está unificado', 'lealez' ) . '</h3><p>' . esc_html__( 'Selecciona una ubicación para administrar sus datos y la conexión con Google en una sola pantalla.', 'lealez' ) . '</p><a class="lealez-btn lealez-btn-primary" href="' . esc_url( $this->page_url( 'locations' ) ) . '">' . esc_html__( 'Ver ubicaciones', 'lealez' ) . '</a></div></div>';
        }
    ...
    private function render_creation_scope_notice() {
        ...
        <div class="lealez-portal lealez-unified-create-notice">
            <div class="lealez-sync-legend">
                <div><span class="lealez-sync-chip is-internal"><?php esc_html_e( 'Guardar en Lealez', 'lealez' ); ?></span><p><?php esc_html_e( 'La ubicación se crea primero en Lealez para que puedas completar y revisar toda su información.', 'lealez' ); ?></p></div>
                <div><span class="lealez-sync-chip is-google-write"><?php esc_html_e( 'Publicar en Google', 'lealez' ); ?></span><p><?php esc_html_e( 'Después de crearla podrás conectar Google, publicar los campos compatibles y revisar su estado desde el mismo perfil.', 'lealez' ); ?></p></div>
            </div>
            <div class="lealez-gmb-guidance is-info"><strong><?php esc_html_e( 'Creación inicial', 'lealez' ); ?></strong><p><?php esc_html_e( 'Crea primero la ubicación y luego continuarás en el perfil completo, organizado por secciones para que puedas diligenciarlo y sincronizarlo con claridad.', 'lealez' ); ?></p></div>
        </div>
        <?php
