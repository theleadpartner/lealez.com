...
public function handle_oauth_callback() {
    ...
    // Success - Now get accounts information immediately
    $accounts = Lealez_GMB_API::get_accounts( $business_id, true );
    
    $summary_message = __( 'Successfully connected to Google My Business!', 'lealez' ) . "\n\n";
    
    if ( ! is_wp_error( $accounts ) && ! empty( $accounts ) ) {
        $account_count = count( $accounts );
        $summary_message .= sprintf( 
            __( '✓ Found %d Google My Business account(s)', 'lealez' ), 
            $account_count 
        ) . "\n";
        ...
        foreach ( $accounts as $account ) {
            if ( isset( $account['accountName'] ) ) {
                $account_names[] = $account['accountName'];
            }
        }
        ...
        if ( ! empty( $account_names ) ) {
            ...
            foreach ( $account_names as $name ) {
                $summary_message .= "  • " . $name . "\n";
            }
        }
        
        $summary_message .= "\n" . __( 'Click "Refresh Locations" in the business page to load your locations data.', 'lealez' );
    } else {
        $summary_message .= "\n" . __( 'Connection successful! Click "Refresh Locations" to load your data.', 'lealez' );
    }
