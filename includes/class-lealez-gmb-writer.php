...
public static function update_location_core( $post_id, $business_id, $location_name, $fields ) {
    ...
    if ( ! class_exists( 'Lealez_GMB_API' ) || ! method_exists( 'Lealez_GMB_API', 'make_writer_request' ) ) {
        $error = new WP_Error(
            'writer_api_unavailable',
            __( '[Writer] Lealez_GMB_API::make_writer_request() no está disponible.', 'lealez' )
        );
        self::log_write_operation( $post_id, array(
            'method'      => $method_key,
            'user_id'     => $user_id,
    ...
    if ( isset( $fields['description'] ) ) {
        ...
        $new_desc = mb_substr( $new_desc, 0, 750 );
    ...
    // ── Llamada HTTP via Lealez_GMB_API::make_writer_request() ──────
    $response = Lealez_GMB_API::make_writer_request( $business_id, 'PATCH', $endpoint, $body, false );
    ...
    if ( is_wp_error( $response ) ) {
        ...
        $source = 'api';
        if ( false !== strpos( (string) $err_code, 'rate_limit' ) ) {
            $source = 'rate_limiter';
        } elseif ( false !== strpos( (string) $err_code, 'token' ) || false !== strpos( (string) $err_code, 'oauth' ) ) {
            $source = 'oauth_token';
        }
...
public static function update_location_hours(
    ...
    // ── Llamada HTTP via make_writer_request ─────────────────────────
    $response    = Lealez_GMB_API::make_writer_request( $business_id, 'PATCH', $endpoint, $body, false );
    $duration_ms = intval( ( microtime( true ) - $start_time ) * 1000 );
...
public static function get_or_create_place_action_link(
    ...
    if ( $existing_link_name ) {
        ...
        $response    = Lealez_GMB_API::make_writer_request( $business_id, 'POST', $endpoint, $body, false );
        $duration_ms = intval( ( microtime( true ) - $start_time ) * 1000 );
...
public static function delete_place_action_link( $post_id, $business_id, $location_name, $link_name ) {
    ...
    $response    = Lealez_GMB_API::make_writer_request( $business_id, 'DELETE', $endpoint, array(), false );
    $duration_ms = intval( ( microtime( true ) - $start_time ) * 1000 );
...
private static function _check_pending_edits( $post_id, $business_id, $location_name ) {
    ...
    try {
        ...
        $get_response = Lealez_GMB_API::make_writer_request( $business_id, 'GET', $endpoint, array(), false );
...
private static function _logger_log( $level, $message, $context = array() ) {
    if ( class_exists( 'Lealez_GMB_Logger' ) && method_exists( 'Lealez_GMB_Logger', 'log' ) ) {
        // Firma real del logger: log( $business_id, $level, $message, $context )
        // El Writer no siempre dispone del business_id en este helper; se pasa 0 como fallback.
        Lealez_GMB_Logger::log( 0, $level, $message, $context );
    } elseif ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
        error_log( '[LEALEZ_WRITER][' . strtoupper( $level ) . '] ' . $message );
    }
}
