...
public function render_metabox( $post ) {
    ...
    $gmb_account_id            = (string) get_post_meta( $post->ID, 'gmb_account_id', true );           // "accounts/123..." o "123" (a veces vacío)
    $gmb_location_id           = (string) get_post_meta( $post->ID, 'gmb_location_id', true );          // "456" o "locations/456"
    $gmb_location_name         = (string) get_post_meta( $post->ID, 'gmb_location_name', true );        // "locations/456" (Business Info API v1) o "accounts/123/locations/456" (si existiera)
    $gmb_location_account_name = (string) get_post_meta( $post->ID, 'gmb_location_account_name', true );// "accounts/123" (si está guardado)

    /**
     * ✅ BACKFILL CRÍTICO:
     * Si gmb_account_id está vacío (tu caso), lo resolvemos así:
     * 1) Si existe gmb_location_account_name => usarlo
     * 2) Si no, resolver desde cache del business (_gmb_locations_available) via Lealez_GMB_API helper
     *
     * Esto permite que:
     * - Media endpoint v4 tenga accountId siempre
     * - Meta "GMB Account ID" quede llenándose (al recargar) sin depender del metabox anterior
     */
    $backfilled_account_resource = '';
    if ( empty( $gmb_account_id ) ) {

        if ( ! empty( $gmb_location_account_name ) ) {
            $backfilled_account_resource = $gmb_location_account_name;
        } elseif ( $parent_business_id && class_exists( 'Lealez_GMB_API' ) && method_exists( 'Lealez_GMB_API', 'resolve_account_resource_name_for_location' ) ) {

            // Usamos location_id si existe, si no usamos location_name
            $location_hint = ! empty( $gmb_location_id ) ? $gmb_location_id : $gmb_location_name;

            $backfilled_account_resource = (string) Lealez_GMB_API::resolve_account_resource_name_for_location(
                $parent_business_id,
                $location_hint,
                $gmb_location_name
            );
        ...
        // Persistir si logramos resolver
        if ( ! empty( $backfilled_account_resource ) ) {
            $gmb_account_id = $backfilled_account_resource;
            update_post_meta( $post->ID, 'gmb_account_id', $gmb_account_id );
        }
    }
    ...
    $debug_payload = array(
        ...
        'meta_gmb_location_account_name' => $gmb_location_account_name,

        // Backfill info
        'backfilled_account_resource_used' => $backfilled_account_resource,
    ...
    /**
     * IMPORTANTE:
     * Para media podemos resolver IDs desde gmb_location_name.
