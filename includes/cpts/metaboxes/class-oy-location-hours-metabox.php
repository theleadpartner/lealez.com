...
public function render_metabox( $post ) {
    ...
    if ( 'hours_metabox_button' === $last_sync_source && ! empty( $last_sync_at ) ) {
        ...
        // Importar Ahora puede poblar visualmente, pero si no se ha presionado el botón,
        // esta auditoría queda vacía. Eso es EXACTAMENTE lo que necesitamos para diferenciar.
    ...
    <div id="oy-hours-sync-bar" style="background:#f0f6fc; border:1px solid #c3d4e4; border-radius:4px; padding:10px 14px; margin-bottom:14px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        ...
        <?php /* ✅ Auditoría visible: indica si este botón ya sincronizó */ ?>
        ...
        $('#oy-hours-sync-btn').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // ✅ Mostrar auditoría real (desde servidor, no desde reloj del navegador)
                    if (d.synced_at) {
...
public function ajax_sync_hours() {
    ...
    // ✅ AUDITORÍA: marcar que ESTA sincronización fue disparada por el botón del metabox de horarios
    $synced_at = current_time( 'mysql' ); // hora WP (config del sitio)
