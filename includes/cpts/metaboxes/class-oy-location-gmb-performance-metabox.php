...
class OY_Location_GMB_Performance_Metabox {
    ...
    private function process_multi_metrics( $raw_response, $requested_metrics ) {
        ...
        /*
         * Real API response structure (fetchMultiDailyMetricsTimeSeries):
         * {
         *   "multiDailyMetricTimeSeries": [
         *     {
         *       "dailyMetricTimeSeries": [      ← nested inner array per group
         *         {
         ...
         * }
         *
         * We iterate the outer array, then the inner dailyMetricTimeSeries array.
         * We also handle a flat structure (dailyMetric at outer level) as a fallback
        ...
        foreach ( $outer_list as $outer_item ) {
            ...
            // Build inner list: handles both nested and flat structures
            if ( isset( $outer_item['dailyMetricTimeSeries'] ) && is_array( $outer_item['dailyMetricTimeSeries'] ) ) {
                $inner_list = $outer_item['dailyMetricTimeSeries'];
            } elseif ( isset( $outer_item['dailyMetric'] ) ) {
                // Flat structure fallback
                $inner_list = array( $outer_item );
            } else {
                continue;
            }
            ...
            foreach ( $inner_list as $item ) {
                ...
                foreach ( $dated_values as $dv ) {
                    ...
                    // 'value' can be integer, string-integer, or absent (means 0).
                    $val = 0;
            ...
            } // end foreach inner (dailyMetricTimeSeries)
        } // end foreach outer (multiDailyMetricTimeSeries)
    ...
    public function ajax_diagnostic() {
        ...
        if ( is_array( $series_items ) && ! empty( $series_items ) ) {
             ...
             * We need to look inside the nested dailyMetricTimeSeries array.
             */
            $outer_first = $series_items[0];
            $info['outer_keys'] = is_array( $outer_first ) ? array_keys( $outer_first ) : 'not an array';

            $inner_series = array();
            if ( isset( $outer_first['dailyMetricTimeSeries'] ) && is_array( $outer_first['dailyMetricTimeSeries'] ) ) {
                $inner_series = $outer_first['dailyMetricTimeSeries'];
            } elseif ( isset( $outer_first['dailyMetric'] ) ) {
                // Flat structure fallback
                $inner_series = array( $outer_first );
            }

            $info['inner_series_count'] = count( $inner_series );

            if ( ! empty( $inner_series ) ) {
                $first = $inner_series[0];
                $dated = isset( $first['timeSeries']['datedValues'] ) ? $first['timeSeries']['datedValues'] : array();
                $info['first_series_metric']      = isset( $first['dailyMetric'] ) ? $first['dailyMetric'] : 'unknown';
                $info['first_series_dated_count'] = count( $dated );
                $info['first_series_sample']      = ! empty( $dated ) ? $dated[0] : 'empty datedValues';
            } else {
                $info['diagnosis'] = 'multiDailyMetricTimeSeries exists but dailyMetricTimeSeries inner array is empty or missing.';
            }
        } else {
